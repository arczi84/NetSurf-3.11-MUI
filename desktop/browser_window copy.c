...
LOG(("browser_window_content_done browser_window_refresh : %p (%s) done",
...
LOG(("browser_window__handle_error: %p (%s) code %d",
		c, content_get_type(c), code));
	if (message == NULL) {
...
{
	assert(bw);
	LOG(("browser_window_destroy_internal: %p", bw));
	browser_window_destroy_children(bw);
	browser_window_destroy_iframes(bw);
	LOG(("browser_window_destroy_internal: %p children destroyed", bw));
	/* Destroy scrollbars */
	if (bw->scroll_x != NULL) {
		scrollbar_destroy(bw->scroll_x);
	}
	LOG(("browser_window_destroy_internal: %p scroll_x destroyed", bw));
	if (bw->scroll_y != NULL) {
		scrollbar_destroy(bw->scroll_y);
	}

	/* clear any pending callbacks */
	LOG(("Clearing browser window browser_window_refresh %p callbacks", bw));
...
{
	LOG(("browser_window_update_extent"));
	if (bw->window != NULL) {
		/* Front end window */
...
nsuerror nsu_getmonotonic_ms2(uint64_t *current_out)
{
    uint64_t current;
		      ...
		      struct browser_window **bw)
{
	...
	nserror err;
	LOG(("Entering browser_window_create"));
	/* Check parameters */
	...
	if (url != NULL) {
		...
		LOG(("browser_window_create: navigating to %s",
		     nsurl_access(url)));
			...
			hlcache_handle *parent)
{
	...
	LOG(("bw %p, url %s, referrer %s, flags 0x%x",
	     bw, nsurl_access(url),
	     (referrer != NULL) ? nsurl_access(referrer) : "NULL",
	     flags));
	...
	LOG(("browser_window_navigate: depth %d", depth));
	...
	LOG(("browser_window_navigate before browser_window_stop: %p", bw));
	browser_window_stop(bw);
	browser_window_remove_caret(bw, false);
	browser_window_destroy_children(bw);
	browser_window_destroy_iframes(bw);
		       ...
		       struct browser_fetch_parameters *params)
{
	...
	NSLOG(netsurf, INFO, "navigate_internal_real: Loading URL: '%s'", nsurl_access(params->url));
	NSLOG(netsurf, DEBUG, "Flags: %x, Referrer: %s", params->flags,
	      params->referrer ? nsurl_access(params->referrer) : "NULL");

	fetch_is_post = (params->post_urlenc != NULL || params->post_multipart != NULL);
	NSLOG(netsurf, DEBUG, "POST: %s", fetch_is_post ? "YES" : "NO");
	...
	if (params->parent_charset != NULL) {
		...
		NSLOG(netsurf, DEBUG, "Child context: charset=%s, quirks=%d",
		      child.charset, child.quirks);
	} else {
		NSLOG(netsurf, DEBUG, "No parent charset context provided.");
	}
	...
	NSLOG(netsurf, DEBUG, "Calling hlcache_handle_retrieve, flags=0x%x", fetch_flags);
	...
	NSLOG(netsurf, DEBUG, "hlcache_handle_retrieve result: %d, handle: %p", res, (void *)c);
...
static nserror
navigate_internal_real1(struct browser_window *bw,
		       struct browser_fetch_parameters *params)
{
	uint32_t fetch_flags = 0;
	bool fetch_is_post;
	...
	case NSERROR_OK:
		...
		if (bw->internal_nav == false) {
			//printf("bw->loading_parameters.url: %p\n", bw->loading_parameters.url);
	...
	default: /* report error to user */
		LOG(("Error retrieving messages_get_errorcode"));
		browser_window_set_status(bw, messages_get_errorcode(res));
		break;
...
{
	int children, index;
LOG(("!!! ABOUT TO CALL browser_window_stop, bw = %p", bw));
LOG(("!!! current_content = %p", bw ? bw->current_content : NULL));
	...
	LOG(( "browser_window_stop: %p browser_window_refresh", bw));
if (bw->current_content != NULL) {
	LOG(("browser_window_stop: scheduling refresh, bw=%p", bw));
	guit->misc->schedule(-1, browser_window_refresh, bw);
}
