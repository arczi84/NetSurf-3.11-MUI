...
static void browser_window_refresh(void *p)
{
    ...
    NSLOG(netsurf, INFO, "browser_window_refresh: disabled for stability");
    
    /* Po prostu anuluj timer i nic nie rób */
    if (bw && guit && guit->misc && guit->misc->schedule) {
        guit->misc->schedule(-1, browser_window_refresh, bw);
    }
    
    return; /* Wyłącz refresh funkcjonalność */
}
static void browser_window_refresh_(void *p)
{
    struct browser_window *bw = p;
    nsurl *url;
    nsurl *refresh;
    ...
    if (bw == NULL) {
        NSLOG(netsurf, ERROR, "browser_window_refresh: bw is NULL");
        return;
    }
    
    /* Check if current_content exists and has proper status */
    if (bw->current_content == NULL) {
        NSLOG(netsurf, ERROR, "browser_window_refresh: current_content is NULL");
        return;
    }
    
    content_status status = content_get_status(bw->current_content);
    if (status != CONTENT_STATUS_READY && status != CONTENT_STATUS_DONE) {
        NSLOG(netsurf, ERROR, "browser_window_refresh: content status is %d (not READY or DONE)", status);
        return;
    }
    
    /* Original assertion - now replaced with proper error checking above */
    /*
    ...
    */
    ...
    if (refresh == NULL) {
        NSLOG(netsurf, DEBUG, "browser_window_refresh: no refresh URL");
        return;
    }
    ...
    NSLOG(netsurf, DEBUG, "browser_window_refresh: navigating to refresh URL");
...
static void browser_window_refresh_org(void *p)
{
	struct browser_window *bw = p;
	nsurl *url;
	nsurl *refresh;
...
static const char *bw_content_msg_name(content_msg type)
{
	switch (type) {
...
browser_window_callback(hlcache_handle *c, const hlcache_event *event, void *pw)
{
	struct browser_window *bw = pw;
	nserror res = NSERROR_OK;
	content_status cstatus = content_get_status(c);
		      ...
		      struct browser_window **bw)
{
	...
	if ((flags & BW_CREATE_HISTORY) == 0) {
		if (((flags & BW_CREATE_CLONE) == 0) || (existing == NULL))
			return NSERROR_BAD_PARAMETER;
	}
	...
	if (url != NULL) {
		...
		(void)browser_window_navigate(ret,
				    url,
				    referrer,
				    nav_flags,
				    NULL,
...
{
	lwc_string *scheme, *path;
	const char *url_str = nsurl_access(params->url);
	const char *ref_str = (params->referrer != NULL) ? nsurl_access(params->referrer) : "(none)";
