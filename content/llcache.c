...
static const char *llcache_event_name(llcache_event_type type)
{
	switch (type) {
					   ...
					   llcache_event *event)
{
	...
	if (event->type != LLCACHE_EVENT_HAD_DATA) {
		write_to_log("llcache: send_event object=%p event=%s users_head=%p\n",
		    (void *)object,
		    llcache_event_name(event->type),
		    (void *)object->users);
	}
	...
	while (user != NULL) {
		...
		if (event->type != LLCACHE_EVENT_HAD_DATA) {
			write_to_log("llcache: send_event dispatch user=%p handle=%p\n", (void *)user, (void *)user->handle);
		}
			...
			llcache_handle **result)
{
	...
	NSLOG(llcache, DEBUG, "Start llcache_handle_retrieve: url=%p, flags=%u, referer=%p, post=%p",
	      url, flags, referer, post);

	/* Perform HSTS transform */
	error = llcache_hsts_transform_url(url, &hsts_url, &hsts_in_use);
	if (error != NSERROR_OK) {
		NSLOG(llcache, DEBUG, "HSTS transform failed: error=%d", error);
		return error;
	}

	NSLOG(llcache, DEBUG, "HSTS transform done: hsts_url=%p, hsts_in_use=%d", hsts_url, hsts_in_use);

	/* Can we fetch this URL at all? */
	if (fetch_can_fetch(hsts_url) == false) {
		NSLOG(llcache, DEBUG, "No fetch handler for URL");
		nsurl_unref(hsts_url);
	...
	if (error != NSERROR_OK) {
		NSLOG(llcache, DEBUG, "llcache_object_user_new failed: error=%d", error);
		nsurl_unref(hsts_url);
		return error;
	}

	NSLOG(llcache, DEBUG, "Created llcache_object_user: %p", user);

	/* Retrieve a suitable object from the cache, creating a new one if needed. */
	error = llcache_object_retrieve(hsts_url, flags, referer, post, 0, hsts_in_use, &object);
	if (error != NSERROR_OK) {
		NSLOG(llcache, DEBUG, "llcache_object_retrieve failed: error=%d", error);
		llcache_object_user_destroy(user);
		nsurl_unref(hsts_url);
		return error;
	}

	NSLOG(llcache, DEBUG, "Retrieved llcache_object: %p", object);
	...
	*result = user->handle;
	...
	NSLOG(llcache, DEBUG, "llcache_handle_retrieve done: result handle=%p", *result);
...
/* Exported interface documented in content/llcache.h */
nserror
llcache_handle_retrieve1(nsurl *url,
			uint32_t flags,
			nsurl *referer,
			const llcache_post_data *post,
			llcache_handle_callback cb, void *pw,
