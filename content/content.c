...
static void content_convert(struct content *c)
{
	...
	if (c->handler->data_complete != NULL) {
		...
		if (strstr(nsurl_access_log(llcache_handle_get_url(c->llcache)), "favicon.ico") != NULL) {
		//printf("favicon.ico fetch aborted\n");
		//	loading_timeout = 1;
		}
}
...
static void content_update_status(struct content *c)
{
	if (c->status == CONTENT_STATUS_LOADING ||
		...
		snprintf(c->status_message, sizeof (c->status_message),
			 ...
			 c->sub_status[0] != '\0' ? ", " : " ",
			 c->sub_status);
			 //loading_timeout = 0; //arczi
			 //printf("content_update_status LOADING**************\n");
	} else {
		snprintf(c->status_message, sizeof (c->status_message),
			 "%s (%.1fs)", messages_get("Done"),
			 (float) c->time / 1000);
			 //printf("content_update_status DONE************** (%.1fs)\n",(float) c->time / 1000);
			 //loading_timeout = 1; //arczi
	}
}
	      ...
	      bool quirks)
{
	...
	/*If url consists favicon text, return*/
//	if (strstr(nsurl_access_log(llcache_handle_get_url(llcache)), "favicon.ico") != NULL) {
	//		return NSERROR_OK;
	//	}
...
void content_destroy(struct content *c)
{
	...
	const nsurl *url = NULL;

	assert(c);
	if (c->llcache != NULL) {
		url = llcache_handle_get_url(c->llcache);
	}
	NSLOG(netsurf, INFO, "content %p %s", c,
	      url != NULL ? nsurl_access_log(url) : "<no-llcache>");
	NSLOG(netsurf, ERROR,
	      "destroying content %p handler %p status %d user_list %p",
	      c, c->handler, c->status, c->user_list);
	if (c->handler == NULL) {
		NSLOG(netsurf, ERROR, "content %p has NULL handler entering destroy", c);
	}
	if (c->user_list == NULL) {
		NSLOG(netsurf, ERROR, "content %p has NULL user_list entering destroy", c);
	}
	if (c->llcache == NULL) {
		NSLOG(netsurf, ERROR, "content %p has NULL llcache entering destroy", c);
	}
	assert(c->locked == false);

	if (c->handler != NULL && c->handler->destroy != NULL)
		c->handler->destroy(c);
	else if (c->handler == NULL) {
		NSLOG(netsurf, ERROR, "content %p skipping handler destroy (NULL handler)", c);
	}

	if (c->llcache != NULL) {
		llcache_handle_release(c->llcache);
	} else {
		NSLOG(netsurf, ERROR, "content %p skipping llcache release (NULL)", c);
	}
	c->llcache = NULL;
	...
	if (c->user_list != NULL) {
		...
		struct content_user *cursor;
		for (cursor = c->user_list; cursor != NULL; cursor = cursor->next) {
			entries++;
		}
		NSLOG(netsurf, ERROR, "freeing user_list for content %p entries %u", c, entries);
		free(c->user_list);
	} else {
		NSLOG(netsurf, ERROR, "content %p user_list already NULL during destroy", c);
	}
	...
	free(c);
	NSLOG(netsurf, ERROR, "content %p freed", c);
}
		 ...
		 void *pw)
{
	...
	NSLOG(netsurf, INFO, "content "URL_FMT_SPC" (%p), user %p %p",
	      ...
	      c, callback, pw);
	NSLOG(netsurf, DEBUG, "add_user details: user_list %p sentinel_next %p",
	      c->user_list,
	      c->user_list != NULL ? (void *)c->user_list->next : NULL);
	...
	if (c->user_list != NULL) {
		...
		struct content_user *cursor;
		for (cursor = c->user_list->next; cursor != NULL; cursor = cursor->next) {
			remaining++;
		}
		NSLOG(netsurf, DEBUG, "users after add: %u", remaining);
	}
		    ...
		    void *pw)
{
	...
	NSLOG(netsurf, INFO, "content "URL_FMT_SPC" (%p), user %p %p",
	      nsurl_access_log(llcache_handle_get_url(c->llcache)),
	      c, callback, pw);
	NSLOG(netsurf, DEBUG, "remove_user details: user_list %p sentinel_next %p",
	      c->user_list,
	      c->user_list != NULL ? (void *)c->user_list->next : NULL);
	...
	if (c->user_list != NULL) {
		...
		NSLOG(netsurf, DEBUG, "users after remove: %u", remaining);
	}
	//printf("content_remove_user **************\n");
	loading_timeout = 1; //arczi
}
...
uint32_t content_count_users(struct content *c)
{
	...
	if (counter == 0) {
		...
		const struct content_user *sentinel = c->user_list;
		const void *first_user = sentinel != NULL ? sentinel->next : NULL;

		if (c->handler != NULL && c->handler->type != NULL) {
			ctype = c->handler->type();
		}
		...
		NSLOG(netsurf, ERROR, "content %p has empty user list (handler %p, status %d, type %d, url %s)",
		      c,
		      c->handler,
		      c->status,
		      ctype,
		      url != NULL ? nsurl_access_log(url) : "<null>");
		NSLOG(netsurf, ERROR, "user_list %p first_user %p llcache %p locked %d active %u",
		      c->user_list,
		      first_user,
		      c->llcache,
		      c->locked,
		...
		NSLOG(netsurf, ERROR, "content_count_users return addresses: %p <- %p",
		      ra0,
		      ra1);
#endif
	}
