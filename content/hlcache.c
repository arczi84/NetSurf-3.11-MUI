...
static void hlcache_clean(void *force_clean_flag)
{
	...
	for (entry = hlcache->content_list; entry != NULL; entry = next) {
		...
		uint32_t users = content_count_users(entry->content);
		if (users != 0)
			continue;
		const llcache_handle *entry_ll = content_get_llcache_handle(entry->content);
		const nsurl *entry_url = NULL;
		if (entry_ll != NULL) {
			entry_url = llcache_handle_get_url(entry_ll);
		}
		NSLOG(netsurf, DEBUG,
		      "hlcache_clean reclaiming entry %p content %p handler %p status %d url %s",
		      entry,
		      entry->content,
		      entry->content != NULL ? entry->content->handler : NULL,
		      entry->content != NULL ? content__get_status(entry->content) : -1,
		      entry_url != NULL ? nsurl_access_log(entry_url) : "<null>");
...
static const char *hlcache_content_msg_name(content_msg msg)
{
	switch (msg) {
		...
		const union content_msg_data *data, void *pw)
{
	...
	write_to_log("hlcache: content_callback content=%p msg=%s handle=%p cb=%p\n", (void *)c,
	    hlcache_content_msg_name(msg), (void *)handle, (void *)handle->cb);
	if (handle->cb != NULL)
		...
		lwc_string *effective_type)
{
	...
	if (entry == NULL) {
		...
		NSLOG(netsurf, DEBUG, "hlcache created content %p (entry %p)",
		      entry->content, entry);
                         ...
                         void *pw)
{
    ...
    NSLOG(llcache, DEBUG, "hlcache_llcache_callback: event type=%d", event->type);

    switch (event->type) {
    case LLCACHE_EVENT_GOT_CERTS:
        NSLOG(llcache, DEBUG, "LLCACHE_EVENT_GOT_CERTS received.");
        if (ctx->handle->cb != NULL) {
            hlcache_event hlevent;
    ...
    case LLCACHE_EVENT_HAD_HEADERS:
        NSLOG(llcache, DEBUG, "LLCACHE_EVENT_HAD_HEADERS received.");
        error = mimesniff_compute_effective_type(
            llcache_handle_get_header(handle, "Content-Type"), NULL, 0,
            ctx->flags & HLCACHE_RETRIEVE_SNIFF_TYPE,
            ctx->accepted_types == CONTENT_IMAGE,
            &effective_type);

        NSLOG(llcache, DEBUG, "Sniff result: error=%d", error);
    ...
    case LLCACHE_EVENT_HAD_DATA:
        NSLOG(llcache, DEBUG, "LLCACHE_EVENT_HAD_DATA received (data len=%zu).", event->data.data.len);
        ...
        if (error != NSERROR_OK) {
            NSLOG(llcache, ERROR, "MIME sniff failed with data: error=%d", error);
    ...
    case LLCACHE_EVENT_DONE:
		NSLOG(llcache, DEBUG, "LLCACHE_EVENT_DONE received.");

		error = mimesniff_compute_effective_type(
			llcache_handle_get_header(handle, "Content-Type"),
			NULL, 0, false, false, &effective_type);

		NSLOG(llcache, DEBUG, "MIME sniff in DONE: error=%d", error);
		...
		NSLOG(llcache, ERROR, "Sending CONTENT_MSG_ERROR due to failed sniffing: code=%d", error);
        ...
        NSLOG(llcache, ERROR, "LLCACHE_EVENT_DONE -> sniff error: code=%d", error);
    ...
    case LLCACHE_EVENT_ERROR:
        NSLOG(llcache, ERROR, "LLCACHE_EVENT_ERROR received: code=%d, msg=%s",
    ...
    case LLCACHE_EVENT_PROGRESS:
        NSLOG(llcache, DEBUG, "LLCACHE_EVENT_PROGRESS received.");
        break;

    case LLCACHE_EVENT_REDIRECT:
        NSLOG(llcache, DEBUG, "LLCACHE_EVENT_REDIRECT received: from=%s to=%s",
              nsurl_access(event->data.redirect.from),
              nsurl_access(event->data.redirect.to));
    ...
    default:
        NSLOG(llcache, WARNING, "Unhandled llcache event type: %d", event->type);
        break;
    }
...
static nserror
hlcache_llcache_callback1(llcache_handle *handle,
			 const llcache_event *event,
			 void *pw)
{
	hlcache_retrieval_ctx *ctx = pw;
...
#include "utils/log.h" // upewnij się, że masz ten include
			...
			hlcache_handle **result)
{
	...
	NSLOG(llcache, DEBUG, "Start hlcache_handle_retrieve: url=%p, flags=%u, referer=%p, post=%p",
	      url, flags, referer, post);

	ctx = calloc(1, sizeof(hlcache_retrieval_ctx));
	if (ctx == NULL) {
		NSLOG(llcache, ERROR, "Failed to allocate hlcache_retrieval_ctx");
		return NSERROR_NOMEM;
	}

	ctx->handle = calloc(1, sizeof(hlcache_handle));
	if (ctx->handle == NULL) {
		NSLOG(llcache, ERROR, "Failed to allocate hlcache_handle");
		free(ctx);
		return NSERROR_NOMEM;
	}

	if (child != NULL) {
		NSLOG(llcache, DEBUG, "Child context provided: charset=%s, quirks=%d",
		      child->charset ? child->charset : "NULL", child->quirks);
		if (child->charset != NULL) {
			ctx->child.charset = strdup(child->charset);
			if (ctx->child.charset == NULL) {
				NSLOG(llcache, ERROR, "Failed to strdup charset");
				free(ctx->handle);
				free(ctx);
				return NSERROR_NOMEM;
			}
	...
	NSLOG(llcache, DEBUG, "Calling llcache_handle_retrieve");
	...
	if (error != NSERROR_OK) {
		NSLOG(llcache, ERROR, "llcache_handle_retrieve failed with code %d", error);
		free((char *) ctx->child.charset);
		free(ctx->handle);
		free(ctx);
	} else {
		NSLOG(llcache, DEBUG, "Retrieved llcache_object: %p", ctx->llcache);
		RING_INSERT(hlcache->retrieval_ctx_ring, ctx);
		*result = ctx->handle;
		NSLOG(llcache, DEBUG, "hlcache_handle_retrieve done: result handle=%p", ctx->handle);
	}
...
/* See hlcache.h for documentation */
nserror
hlcache_handle_retrieve1(nsurl *url,
			uint32_t flags,
			nsurl *referer,
			llcache_post_data *post,
			hlcache_handle_callback cb, void *pw,
