...
//	while((opt = getopt_long(argc, argv, "f:b:w:h:",long_options, &option_index)) != -1) {
...
main(int argc, char** argv)
{
	...
	respaths = fb_init_resource_path("Resources");
...
//	free(options);
	...
	nsfb = framebuffer_initialise("sdl"/*fename*/, fewidth, feheight, febpp);
	if (nsfb == NULL)
	...
	//printf("nsurl_create returned  %d\n", ret);
	if (ret == NSERROR_OK) {
	...
	//printf("browser_window_create returned 1\n");
	if (ret != NSERROR_OK) {
			//printf("browser_window_create returned 2\n");
		fb_warn_user("Errorcode:", messages_get_errorcode(ret));
	} else {
		//printf("browser_window_create returned 3\n");
		framebuffer_run();
    //printf("browser_window_create returned 4\n");
