...
{
	GETDATA;
	return DoMethodA(data->hotlistwin, msg);
}
...
DEFSMETHOD(Application_DownloadDone)
{
	...
	return DoMethodA(data->dlwin, msg);
}
...
DEFSMETHOD(Window_MenuAction)
{
	...
	if (data->active_window)
		DoMethodA(data->active_window->winobj, msg);
...
{
	GETDATA;
	return DoMethodA(data->active_window->winobj, msg);
}
