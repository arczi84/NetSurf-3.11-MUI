...
/**
 * Send a callback message to the fetch system
 */
static void mui_fetch_file_send_callback(fetch_msg_type msg_type, struct fetch_info *fetch, const void *data, size_t data_len)
{
	fetch_msg msg;
	...
	switch (msg_type) {
		...
		case FETCH_DATA:
			...
			break;
		case FETCH_ERROR:
			msg.data.error = (const char *)data;
			break;
		case FETCH_FINISHED:
		case FETCH_PROGRESS:
		default:
			/* For other message types, data might not be used */
			break;
	}
 ...
 * FETCH_HEADER, with the Content-Type header in data, then one or more times
	...
	if (fetch) {
		...
		if (fetch->path) {
			...
			if (fetch->fh) {
				...
				mui_fetch_file_send_callback(FETCH_HEADER, fetch, fetch->mimetype, strlen(fetch->mimetype));
				ok = 1;
			} else {
				STRPTR errorstring;
				ULONG length;
				...
				if (errorstring) {
					...
					mui_fetch_file_send_callback(FETCH_ERROR, fetch, errorstring, 0);
					fetch->aborted = true;
...
static void mui_fetch_file_poll(const char *scheme_ignored)
{
	...
	ITERATELISTSAFE(fetch, next, &file_fetcher_list) {
		...
		if (!fetch->aborted) {
			...
			mui_fetch_file_send_callback(FETCH_DATA, fetch, fetch_buffer, len);

			if (len < sizeof(fetch_buffer) && !fetch->aborted) {
				mui_fetch_file_send_callback(FETCH_FINISHED, fetch, &fetch->cachedata, 0);
				fetch->aborted = TRUE;
			}
		}
