			  ...
			  lwc_string *mime_type)
{
	...
	if (fh == 0) {
		free(fname);
		NSLOG(netsurf, INFO, "Open(): %s", strerror(errno));
		return NSERROR_SAVE_FAILED;
	}

	Write(fh, (APTR)data, data_len);
	Close(fh);
...
void mui_save_complete_init(void)
{
...
/* Documented in save_complete.h */
nserror mui_save_complete_finalise(void)
{
...
/* Documented in save_complete.h */
nserror
mui_save_complete(hlcache_handle *c,
	      const char *path,
	      save_complete_set_type_cb set_type)
{
	nserror result;
