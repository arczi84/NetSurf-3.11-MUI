...
{
    LOG(("DEBUG: transferanimclass doset ENTRY"));
    LOG(("DEBUG: data=%p, obj=%p, taglist=%p", data, obj, taglist));
    ...
    LOG(("DEBUG: Starting tag iteration"));
    
    while ((tag = NextTagItem(&tstate)) != NULL)
    {
        LOG(("DEBUG: Processing tag 0x%08x with data 0x%08x", tag->ti_Tag, tag->ti_Data));
        
        ULONG tag_data = tag->ti_Data;
        switch (tag->ti_Tag)
        {
            case MA_TransferAnim_Animate:
                LOG(("DEBUG: MA_TransferAnim_Animate case"));
                LOG(("DEBUG: tag_data=%lu, data->animate=%d", tag_data, data->animate));
                LOG(("DEBUG: data->is_shown=%d, data->added=%d", data->is_shown, data->added));
                ...
                if (tag_data && data->animate == 0)
                {
                    ...
                    {
                        LOG(("DEBUG: About to add input handler"));
                        LOG(("DEBUG: application=%p", application));
                        LOG(("DEBUG: data->ihnode=%p", &data->ihnode));
                        
                        if (!application) {
                            LOG(("ERROR: application is NULL - cannot add input handler"));
                            break;
                        }
                        
                        data->added = TRUE;
                        
                        LOG(("DEBUG: Calling DoMethod(application, MUIM_Application_AddInputHandler)"));
                        ULONG result = DoMethod(application, MUIM_Application_AddInputHandler, (ULONG)&data->ihnode);
                        LOG(("DEBUG: DoMethod returned %lu", result));
                    } else {
                        LOG(("DEBUG: Skipping input handler - is_shown=%d, added=%d", data->is_shown, data->added));
                    }
                }
                else if (!tag_data && data->animate)
                {
                    ...
                    if (data->is_shown && data->added)
                    {
                        LOG(("DEBUG: About to remove input handler"));
                        
                        if (!application) {
                            LOG(("ERROR: application is NULL - cannot remove input handler"));
                            break;
                        }
                        
                        ULONG result = DoMethod(application, MUIM_Application_RemInputHandler, (ULONG)&data->ihnode);
                        LOG(("DEBUG: RemInputHandler returned %lu", result));
                        
                        data->imagenum = 0;
                        data->added = FALSE;
                        
                        LOG(("DEBUG: Calling MUI_Redraw"));
            ...
            default:
                LOG(("DEBUG: Unknown tag 0x%08x", tag->ti_Tag));
                break;
        }
    }
    
    LOG(("DEBUG: transferanimclass doset completed"));
}
STATIC VOID doset1(struct Data *data, APTR obj, struct TagItem *taglist)
{
