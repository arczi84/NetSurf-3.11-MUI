...
extern int loading_timeout; /* timeout in miliseconds */
...
static void framebuffer_run(void)
{
	...
	while (fb_complete != true) {
		...
		if (fbtk_get_redraw_pending(fbtk) || nsoption_bool(warp_mode) || (loading_timeout == 0))
			timeout = 0;
...
//	browser_window_reformat(gw->bw, false, width, height);
	browser_window_schedule_reformat(gw->bw);  //needed for resize
}
...
gui_window_get_dimensions(struct gui_window *gw, int *width, int *height)
{
	...
	*height = fbtk_get_height(gw->browser);
	...
	LOG(("gui_window_get_dimensions() called, width=%d, height=%d",
			*width, *height));
