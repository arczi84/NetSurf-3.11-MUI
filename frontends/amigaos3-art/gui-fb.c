       ...
       struct browser_window *bw)
{
	...
	NSLOG("panning %d, %d", bwidget->panx, bwidget->pany);
...
fb_browser_window_redraw(fbtk_widget_t *widget, fbtk_callback_info *cbi)
{
	...
	if (bwidget == NULL) {
		NSLOG("browser widget from widget %p was null", widget);
		return -1;
	}
...
process_cmdline(int argc, char** argv)
{
	...
	while((opt = getopt_long2(argc, argv, "f:b:w:h:",
...
static nserror set_defaults(struct nsoption_s *defaults)
{
	...
	if (nsoption_charp(cookie_file) == NULL ||
		...
		NSLOG("Failed initialising cookie options");
...
fb_browser_window_click(fbtk_widget_t *widget, fbtk_callback_info *cbi)
{
	...
	NSLOG("browser window clicked at %d,%d", cbi->x, cbi->y);
...
fb_browser_window_input(fbtk_widget_t *widget, fbtk_callback_info *cbi)
{
	...
	NSLOG("got value %d", cbi->event->value.keycode);
	       ...
	       const char *toolbar_layout)
{
	...
	NSLOG("Using toolbar layout %s", toolbar_layout);
	...
	while ((itmtype >= toolbar_layout) && 
		...
		NSLOG("toolbar adding %c", *itmtype);
		...
		default:
			widget = NULL;
			xdir = 0;
			NSLOG("Unknown element %c in toolbar layout", *itmtype);
		        break;
...
create_normal_browser_window(struct gui_window *gw, int furniture_width)
{
	...
	NSLOG("status bar %p at %d,%d", gw->status, fbtk_get_absx(gw->status), fbtk_get_absy(gw->status));
...
main(int argc, char** argv)
{
	...
	const char *current_user_dir = strdup("PROGDIR:Resources/");	
#ifdef CACHE2
    current_user_cache = strdup("PROGDIR:Resources/Cache");
#endif
	...
	NSLOG("calling browser_window_create");
	...
	if (fb_font_finalise() == false)
		NSLOG("Font finalisation failed.");
