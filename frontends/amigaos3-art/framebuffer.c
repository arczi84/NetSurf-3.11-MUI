 ...
 */
static nserror
framebuffer_plot_text_ttf(const struct redraw_context *ctx,
 ...
 */
static nserror
framebuffer_plot_text_internal(const struct redraw_context *ctx,
		const struct plot_font_style *fstyle,
		int x,
		int y,
		const char *text,
		...
		size_t length)
{
	if (nsoption_bool(bitmap_fonts)){
		framebuffer_plot_text_internal(ctx, fstyle, x,  y, text, length);
	}
	else {
		framebuffer_plot_text_ttf(ctx, fstyle, x,  y, text, length);
	}	
	
}

struct plotter_table fb_plotters = {
...
struct plotter_table fb_plotters_internal = {
	.clip = framebuffer_plot_clip,
	.arc = framebuffer_plot_arc,
	.disc = framebuffer_plot_disc,
	.line = framebuffer_plot_line,
	...
	//.path = framebuffer_plot_path,
	.bitmap = framebuffer_plot_bitmap,
	.text = framebuffer_plot_text_internal,
	.option_knockout = true,
};
