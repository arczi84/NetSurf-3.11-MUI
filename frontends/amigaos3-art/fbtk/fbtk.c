...
#ifdef FBTK_LOGGING
	NSLOG("redrawing %p %d,%d %d,%d", widget, widget->redraw.x, widget->redraw.y, widget->redraw.width, widget->redraw.height);
#endif
...
{
	NSLOG("setting mapping on %p to %d", widget, map);
	widget->mapped = map;
	if (map) {
...
fbtk_get_root_widget(fbtk_widget_t *widget)
{
	...
	if (widget->type != FB_WIDGET_TYPE_ROOT) {
		NSLOG("Widget with null parent that is not the root widget!");
		return NULL;
	}
...
#ifdef FBTK_LOGGING
	NSLOG("creating %p %d,%d %d,%d", neww, x, y, width, height);
#endif
...
#ifdef FBTK_LOGGING
	NSLOG("using %p %d,%d %d,%d", neww, x, y, width, height);
#endif
	/* set values */
	neww->type = type;
	neww->x = x;
...
#ifdef FBTK_LOGGING
		NSLOG("clipping %p %d,%d %d,%d", widget, plot_ctx.x0, plot_ctx.y0, plot_ctx.x1, plot_ctx.y1);
#endif
