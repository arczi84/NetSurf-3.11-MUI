...
/*8000;//px_to_pt(*font_height * PLOT_STYLE_SCALE);*/
...
text_input(fbtk_widget_t *widget, fbtk_callback_info *cbi)
{
	...
	//if (nsoption_bool(bitmap_fonts))
	//	fb_font_width = fb_font_width_internal;
	...
	//NSLOG("(fbtk) got value %d", cbi->event->value.keycode);
	...
	case NSFB_KEY_MOUSE_5:    
 	case NSFB_KEY_LALT:
	case NSFB_KEY_RALT:	/*Don't act on LALT and RALT*/
	...
	default:
		if (modifier & FBTK_MOD_LCTRL || modifier & FBTK_MOD_RCTRL) {
			/* AMIGA pressed, don't enter any text */
			if (value == NSFB_KEY_u) {
				/* AMIGA+U: clear writable */
				widget->u.text.idx = 0;
				widget->u.text.len = 0;
				widget->u.text.text[widget->u.text.len] = '\0';
			...
			else if (value == NSFB_KEY_v) {
				widget->u.text.text = strdup(ReadClip());
				widget->u.text.idx = strlen(ReadClip());
				widget->u.text.len = strlen(ReadClip());
				widget->u.text.width = strlen(ReadClip());
				widget->u.text.text[widget->u.text.len] = '\0';	
...
fbtk_set_text(fbtk_widget_t *widget, const char *text)
{
	...
	{
		fb_font_width_internal(&font_style, widget->u.text.text,
			widget->u.text.len, &widget->u.text.width);
		fb_font_width_internal(&font_style, widget->u.text.text,
			widget->u.text.idx, &widget->u.text.idx_offset);		
	}
