...
set_palette(nsfb_t *nsfb)
{
    ...
    for (loop = 0; loop < 256; loop++) {
        palette[loop].r = (int_fast16_t)(nsfb->palette->data[loop]      ) & 0xFF;
        palette[loop].g = (int_fast16_t)(nsfb->palette->data[loop] >>  8) & 0xFF;
        palette[loop].b = (int_fast16_t)(nsfb->palette->data[loop] >>  16) & 0xFF;
...
static bool sdl_input(nsfb_t *nsfb, nsfb_event_t *event, int timeout)
{
	...
	if (nsoption_bool(warp_mode) || (loading_timeout == 0)) 
		timeout = 0;
...
#ifdef RTG	
	if (sdlevent.motion.x < 3) {
		SDL_ShowCursor(SDL_ENABLE);
		//Printf("x == %ld   ", sdlevent.motion.x);
		//Printf("y == %ld \n", sdlevent.motion.y);
