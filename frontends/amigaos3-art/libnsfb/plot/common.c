...
static inline nsfb_colour_t pixel_to_colour_argb(UNUSED nsfb_t *nsfb, uint32_t pixel)
{
...
#ifdef __BIG_ENDIAN_BGRA__
				abpixel = Swap32(pixel[yoff + xoff]);
#else				
       ...
       bool alpha)
{
        ...
        if (alpha) {
                for (yloop = yoff; yloop < height; yloop += bmp_stride) {
                        for (xloop = 0; xloop < width; xloop++) {		
#ifdef __BIG_ENDIAN_BGRA__						
                            abpixel = Swap32(pixel[yloop + xloop + xoff]);
#else								
...
static bool readrect(nsfb_t *nsfb, nsfb_bbox_t *rect, nsfb_colour_t *buffer)
{
	...
	{
				if (order == 11)	
						*buffer = pixel_to_colour_argb(nsfb, *(pvideo + xloop));					
				else
