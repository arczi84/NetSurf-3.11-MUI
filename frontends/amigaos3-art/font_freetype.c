...
fb_new_face(const char *option, const char *resname, const char *fontname)
{
	...
	char buf[PATH_MAX];
        ...
        if (option != NULL) {
                ...
                newf->fontfile = strdup(fontname);
        }

        error = FTC_Manager_LookupFace(ft_cmanager, (FTC_FaceID)newf, &aface);
        if (error) {
                NSLOG(netsurf, INFO,"Could not find font face %s (code %d)", fontname, error);
                free(newf->fontfile);
                free(newf);
                newf = NULL;
        }
...
bool fb_font_init(void)
{
	...
	fb_face = fb_new_face(nsoption_charp(fb_face_sans_serif_bold),
                            "PROGDIR:Fonts/DejaVuSans-Bold.ttf",
	...
	fb_face = fb_new_face(nsoption_charp(fb_face_monospace_bold),
			      "PROGDIR:Fonts/DejaVuSansMono-Bold.ttf",
...
static void fb_fill_scalar(const plot_font_style_t *fstyle, FTC_Scaler srec)
{
	...
	srec->pixel = 0;
#if defined AGA
	srec->x_res = srec->y_res = nsoption_int(browser_dpi)-5;//90;//browser_get_dpi();
#else
	srec->x_res = srec->y_res = nsoption_int(browser_dpi);
#endif
}
...
{
	if (nsoption_bool(bitmap_fonts)){
		fb_font_position_internal(fstyle,
		string,length,
